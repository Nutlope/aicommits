...
const httpsPost = async (
	...
	new Promise<{
		...
		request.on('timeout', () => {
			...
			reject(
				new KnownError(
					`Time out error: request took over ${timeout}ms. Try increasing the \`timeout\` config, or checking the OpenRouter API status https://status.openrouter.ai`
				)
			);
		});
...
const createChatCompletion = async (
	...
	if (
		...
		if (response.statusCode === 500) {
			errorMessage += '\n\nCheck the API status: https://status.openrouter.ai';
		}
