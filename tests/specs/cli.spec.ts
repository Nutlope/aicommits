...
describe('cli', () => {
	...
	it.concurrent('Generates message with gitmoji', async ({ expect }) => {
		const fixture = await createFixture({
			'README.md': '',
		});
	...
	it.concurrent('Generates message with conventional commits & gitmoji', async ({ expect }) => {
		const fixture = await createFixture({
			'README.md': '',
		});
	...
	it.concurrent('Generates test: convential commit message', async ({ expect }) => {
		const fixture = await createFixture({
			'aicommits.spec.ts': '',
		});
	...
	it.concurrent('Generates (docs|chore): convential commit message', async ({ expect }) => {
		const fixture = await createFixture({
			'README.md': '',
		});
	...
	it.concurrent('Generates (ci|build): convential commit message', async ({ expect }) => {
		const fixture = await createFixture({
			'.github': {
				workflows: {
					'deploy.yml': `
...
`,
	...
	it.concurrent('Generates (change|fix): convential commit message', async ({ expect }) => {
		const fixture = await createFixture({
